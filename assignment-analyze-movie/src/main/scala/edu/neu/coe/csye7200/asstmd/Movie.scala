...
object Movie extends App {
  ...
  // Instantiate the Ingest class for Movie
  val ingester = new Ingest[Movie]()
  // Load the movie metadata from the resources folder
  val source = Source.fromResource("movie_metadata.csv")
  // Read and parse the movie data into a list
  val moviesTry = ingester(source).toList

  // Extract IMDb scores
  val scores = moviesTry.flatMap(_.toOption).map(_.reviews.imdbScore)

  // Calculate mean IMDb score
  val meanScore = if (scores.nonEmpty) scores.sum / scores.size else 0.0

  // Calculate the sum of the squared differences from the mean
  val sumOfSquares = scores.foldLeft(0.0)((total, score) => total + math.pow(score - meanScore, 2))

  // Calculate the mean variance
  val meanVariance = sumOfSquares / scores.size

  // Calculate the standard deviation
  val standardDeviation = math.sqrt(meanVariance)

  println(s"Mean IMDb Rating Score: $meanScore")
  println(s"Standard Deviation of IMDb Rating Score: $standardDeviation")
...
object Rating {
  ...
  def apply(s: String): Rating = s match {
    case rRating(code, _, num) => Rating(code, Option(num).map(_.toInt))
